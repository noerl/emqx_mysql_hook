...
register_metrics() ->
    [emqx_metrics:new(MetricName) || MetricName <- ['mysql_hook.client_connected',
                                                    'mysql_hook.client_disconnected',
                                                    'mysql_hook.client_subscribe',
                                                    'mysql_hook.client_unsubscribe',
                                                    'mysql_hook.session_created',
                                                    'mysql_hook.session_subscribed',
                                                    'mysql_hook.session_unsubscribed',
                                                    'mysql_hook.session_terminated',
                                                    'mysql_hook.message_publish',
                                                    'mysql_hook.message_deliver',
                                                    'mysql_hook.message_acked']].
...
on_client_connected(#{client_id := ClientId, username := Username}, 0, ConnInfo, _Env) ->
    emqx_metrics:inc('mysql_hook.client_connected'),
...
on_client_disconnected(#{client_id := ClientId, username := Username}, Reason, _Env)
    when is_atom(Reason) ->
    emqx_metrics:inc('mysql_hook.client_disconnected'),
...
on_client_subscribe(#{client_id := ClientId, username := Username}, TopicTable, {Filter}) ->
    lists:foreach(fun({Topic, Opts}) ->
      with_filter(
        fun() ->
          emqx_metrics:inc('mysql_hook.client_subscribe'),
...
on_client_unsubscribe(#{client_id := ClientId, username := Username}, TopicTable, {Filter}) ->
    lists:foreach(fun({Topic, Opts}) ->
      with_filter(
        fun() ->
          emqx_metrics:inc('mysql_hook.client_unsubscribe'),
...
on_session_created(#{client_id := ClientId}, SessInfo, _Env) ->
    emqx_metrics:inc('mysql_hook.session_created'),
...
on_session_subscribed(#{client_id := ClientId}, Topic, Opts, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('mysql_hook.session_subscribed'),
...
on_session_unsubscribed(#{client_id := ClientId}, Topic, _Opts, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('mysql_hook.session_unsubscribed'),
...
on_session_terminated(#{client_id := ClientId}, Reason, _Env) when is_atom(Reason) ->
    emqx_metrics:inc('mysql_hook.session_terminated'),
...
on_message_publish(Message = #message{topic = Topic, flags = #{retain := Retain}}, {Filter}) ->
    ...
    with_filter(
      fun() ->
        emqx_metrics:inc('mysql_hook.message_publish'),
...
on_message_deliver(#{client_id := ClientId, username := Username}, Message = #message{topic = Topic, flags = #{retain := Retain}}, {Filter}) ->
  with_filter(
    fun() ->
      emqx_metrics:inc('mysql_hook.message_deliver'),
      {FromClientId, FromUsername} = format_from(Message),
...
on_message_acked(#{client_id := ClientId}, Message = #message{topic = Topic, flags = #{retain := Retain}}, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('mysql_hook.message_acked'),
        {FromClientId, FromUsername} = format_from(Message),
